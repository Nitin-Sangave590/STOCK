    ...
    <div class="table-responsive">
        <table class="table table-striped table-bordered">
            ...
            while ($row = $result_purchase->fetch_assoc()) {
                ...
                $total_purchase += $row['weight'];
            }
            while ($row = $result_sales->fetch_assoc()) {
                $combined_results[] = $row;
                $total_sales += $row['weight'];
            }

            while ($row = $result_purchase->fetch_assoc()) {
                $combined_results[] = $row;
                $total_purchase += $row['hamali'];
            }
            while ($row = $result_sales->fetch_assoc()) {
                $combined_results[] = $row;
                $total_sales += $row['hamali'];
            }

             while ($row = $result_purchase->fetch_assoc()) {
                $combined_results[] = $row;
                $total_purchase += $row['freight'];
            }
            while ($row = $result_sales->fetch_assoc()) {
                $combined_results[] = $row;
                $total_sales += $row['freight'];
            }

              while ($row = $result_purchase->fetch_assoc()) {
                $combined_results[] = $row;
                $total_purchase += $row['qty'];
            }
            while ($row = $result_sales->fetch_assoc()) {
                $combined_results[] = $row;
                $total_sales += $row['freight'];
            ...
            <tfoot>
                ...
                <tr class="fw-bold">
                    ...
                    <td><?= number_format($total_sales - $total_purchase, 2) ?></td>
                    <td><?= number_format($total_sales - $total_purchase, 2) ?></td>
                </tr>
            </tfoot>
        </table>
