            ...
            <tbody>
                ...
                $stmt = $conn->prepare("
                    SELECT 
                        i.id, 
                        i.name, 
                        COALESCE((SELECT SUM(quantity) FROM purchase_details WHERE item_id = i.id), 0) AS total_purchasedCOALESCE((SELECT SUM(quantity) FROM purchase_details WHERE item_id = i.id), 0) AS total_purchased,
