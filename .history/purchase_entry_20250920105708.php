...
$stmt = $conn->prepare("SELECT id, name FROM accounts WHERE type = ? ORDER BY name DESC");
$type = 'supplier';
...
<div class="container mt-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="input-group w-50">
            <input type="text" class="form-control" id="searchInput" placeholder="Search by Supplier or Invoice..." aria-label="Search purchases">
    ...
    <table class="table table-hover" id="purchaseTable">
        ...
        <tbody>
            ...
            $limit = 10; // Assuming a limit of 10 records per page
            $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
            $offset = ($page - 1) * $limit;

            $query = "SELECT p.*, a.name AS supplier 
                     FROM purchases p 
                     JOIN accounts a ON p.supplier_id = a.id";
            
            if (!empty($search)) {
                $query .= " WHERE a.name LIKE ? OR p.invoice_number LIKE ?";
                $search_param = "%" . $search . "%";
            }
            ...
            $count_query = "SELECT COUNT(*) as total FROM purchases p JOIN accounts a ON p.supplier_id = a.id";
            if (!empty($search)) {
                $count_query .= " WHERE a.name LIKE ? OR p.invoice_number LIKE ?";
