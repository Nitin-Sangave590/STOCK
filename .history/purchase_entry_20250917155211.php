...
if ($action === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$supplier_id || !$invoice_number || !$purchase_date) {
        ...
        if (!$stmt->execute()) {
            ...
            // Insert purchase details and calculate product total
            ...
            if ($modal_type !== 'error') {
                $net_total = $subtotal - $hamali - $freight - $uchal; // Net amount after deductions
    ...
    if (!$purchase_id) {
        ...
        if (!$supplier_id || !$invoice_number || !$purchase_date) {
            ...
            if (!$stmt->execute()) {
                ...
                // Insert new purchase details and calculate product total
                if (!empty($_POST['item_id'])) {
                    foreach ($_POST['item_id'] as $index => $item_id) {
                        $item_id = intval($item_id);
                        $quantity = floatval($_POST['quantity'][$index] ?? 0);
...
<div class="container mt-4">
    ...
    <div id="purchaseFormContainer" class="card p-4 shadow mb-4" style="display:<?= ($action === 'edit') ? 'block' : 'none' ?>;">
        <form method="POST" id="purchaseForm" action="purchase_entry.php?action=<?= ($action === 'edit') ? 'update&id=' . intval($_GET['id']) : 'add' ?>">
            ...
            <!-- Product Total and Net Amount -->
            <div class="row mb-3">
                <div class="col-md-6">
                    <label class="form-label">Product Total</label>
                    <input type="number" step="0.01" id="productTotal" class="form-control fw-bold" readonly>
                </div>
                <div class="col-md-6">
                    <label class="form-label">Net Amount</label>
                    <input type="number" step="0.01" id="netAmount" class="form-control fw-bold" readonly>
                </div>
            </div>
...
$(document).ready(function() {
    ...
    function calculateTotals() {
        ...
        let netAmount = productTotal - hamali - freight - uchal;
        $('#productTotal').val(productTotal.toFixed(2));
