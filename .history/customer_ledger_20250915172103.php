...
<head>
    ...
    <style>
        .ledger-modal .modal-dialog { max-width: 95%; }
        .ledger-modal .modal-content { border-radius: 12px; border: 2px solid #6f42c1; }
        .ledger-modal .modal-header { background: #6f42c1; color: white; font-weight: bold; }
        .ledger-table th, .ledger-table td { text-align: center; vertical-align: middle; }
        .ledger-table thead { background: #f8f9fa; }
        .ledger-table tfoot { background: #f3e8ff; font-weight: bold; }
    </style>
</head>
<body class="p-4">
...
<table class="table table-bordered table-hover">
    ...
    <tbody>
        <?php while ($cust = $customers->fetch_assoc()): ?>
            <?php
            $stmt1 = $conn->prepare("SELECT COALESCE(SUM(total_amount),0) FROM sales WHERE customer_id=?");
            $stmt1->bind_param("i", $cust['id']);
            $stmt1->execute();
            $stmt1->bind_result($sales_total);
            $stmt1->fetch();
            $stmt1->close();

            $stmt2 = $conn->prepare("SELECT COALESCE(SUM(amount),0) FROM customer_receipts WHERE customer_id=?");
            $stmt2->bind_param("i", $cust['id']);
            $stmt2->execute();
            $stmt2->bind_result($receipts_total);
            $stmt2->fetch();
            $stmt2->close();
...
if (isset($_GET['ajax']) && $_GET['ajax'] == '1') {
    ...
    // Fetch full sales details
    $stmt1 = $conn->prepare("SELECT invoice_number, bill_no, total_amount, `date`, hamali, freight 
    ...
    $stmt2 = $conn->prepare("SELECT amount, `date`, description FROM customer_receipts 
                             WHERE customer_id=? AND `date` BETWEEN ? AND ? ORDER BY `date` ASC");
    ...
    <table class="table table-bordered ledger-table">
        <thead>
            ...
            <tr>
                <th>Date</th><th>Bill No</th><th>Invoice</th><th>Amount</th>
        ...
        <tbody>
            <?php while ($row = $sales->fetch_assoc()): ?>
                ...
                <?php $total_debit += $row['total_amount']; ?>
            <?php endwhile; ?>
        ...
        <tfoot>
            <tr>
                <td colspan="4" class="text-start">Total Debit: <?= number_format($total_debit,2) ?></td>
                <td colspan="3" class="text-end">Total Credit: <?= number_format($total_credit,2) ?></td>
            </tr>
            <tr>
                <td colspan="7" class="text-end">
                    <strong>Closing Balance: <?= number_format($total_debit - $total_credit,2) ?></strong>
                </td>
            </tr>
        </tfoot>
    </table>
