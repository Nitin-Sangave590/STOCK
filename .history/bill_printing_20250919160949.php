    ...
    if (!empty($selected_ids)) {
        ...
        if ($stmt_details) {
            ...
            foreach ($selected_ids as $id) {
                ...
                foreach ($purchase_details as $detail) {
                    $whatsapp_msg .= "Product: " . htmlspecialchars($detail['item_name'] ?? 'N/A') . ", Qty: " . ($detail['quantity'] ?? 0) . ", Weight: " . ($detail['weight'] ?? 0) . ", Rate: ₹" . number_format($detail['rate'] ?? 0, 2) . ", Total: ₹" . number_format($detail['total'] ?? 0, 2) . "\n";
                }
                $product_total = array_sum(array_column(array_values($purchase_details), 'total'));
                $total_expenses = ($purchase['hamali'] ?? 0) + ($purchase['freight'] ?? 0) + ($purchase['uchal'] ?? 0);
                $net_amount = $product_total - $total_expenses;
        ...
        $modal_message = "Please select at least one patti.";
        $modal_type = 'error';
    }
}
...
<div class="modal fade no-print" id="successModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <a href="bill_printing.php?page=<?= $page ?>" class="btn btn-primary">Back to Patti List</a>
            </div>
        </div>
    </div>
</div>
...
$(document).ready(function() {
    ...
    $('#previewBtn').click(function() {
        ...
        if (selectedIds.length === 0) {
            $('#errorMessage').text('Please select at least one patti.');
            $('#errorModal').modal('show');
            return;
        }
    ...
    function submitForm(ids, action) {
        let form = $('<form>').attr({ method: 'POST', action: 'bill_printing.php' });
