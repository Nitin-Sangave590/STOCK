...
if (isset($_POST['preview']) || isset($_POST['print'])) {
    ...
    if (!empty($selected_ids)) {
        ...
        foreach ($selected_ids as $id) {
            ...
            $preview_content .= '<table><thead><tr>';
            ...
            foreach ($purchase_details as $detail) {
                $preview_content .= '<tr>';
                $preview_content .= '<td>' . htmlspecialchars($detail['item_name']) . '</td>';
                $preview_content .= '<td>' . $detail['quantity'] . '</td>';
                $preview_content .= '<td>' . $detail['weight'] . '</td>';
                $preview_content .= '<td>' . number_format($detail['rate'], 2) . '</td>';
                $preview_content .= '<td>' . number_format($detail['total'], 2) . '</td>';
                $preview_content .= '</tr>';
            }
            ...
            $preview_content .= '</div>';
            $preview_content .= '<div class="invoice-header"><h5>!!---Thank You Visit Again---!!</h5></div>';
            $preview_content .= '</div>';
        }
    }
}
...
<head>
    ...
    <style>
        ...
        .print-container { max-width: 800px; margin: auto; background: #fff; padding: 25px 40px; border-radius: 12px; box-shadow: 0px 4px 12px rgba(0,0,0,0.1); }
...
<div class="modal fade" id="successModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <a href="patti_register.php?page=<?= $page ?>" class="btn btn-primary">Back to Patti List</a>
            </div>
        </div>
    </div>
</div>
...
$(document).ready(function() {
    ...
    $('#previewBtn').click(function() {
        ...
        $.post('patti_register.php', { selected_ids: selectedIds, preview: true, start_date: $('[name="start_date"]').val(), end_date: $('[name="end_date"]').val(), page: <?= $page ?> }, function(response) {
    ...
    $('#printBtn').click(function() {
        ...
        $('#printPreviewModal').modal('show'); // Mandatory preview before print
    });
    ...
    $('#printNowBtn').click(function() {
        ...
        $.post('patti_register.php', { selected_ids: selectedIds, print: true, start_date: $('[name="start_date"]').val(), end_date: $('[name="end_date"]').val(), page: <?= $page ?> }, function(response) {
            window.print();
    ...
    $('#whatsappBtn').click(function() {
        ...
        $.post('patti_register.php', { selected_ids: selectedIds, whatsapp: true, start_date: $('[name="start_date"]').val(), end_date: $('[name="end_date"]').val(), page: <?= $page ?> }, function(response) {
