...
if ($action == 'add' && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $conn->query("INSERT INTO purchases (supplier_id, invoice_number, purchase_date, total_amount) 
                  VALUES ($supplier_id, '$invoice_number', '$purchase_date', 0)");
    ...
    for ($i = 0; $i < count($_POST['item_id']); $i++) {
        ...
        $conn->query("INSERT INTO purchase_details (purchase_id, item_id, quantity, weight, rate, total) 
                      VALUES ($purchase_id, $item_id, $quantity, $weight, $rate, $total)");

        // Optionally update stock in items table
        $conn->query("UPDATE items SET stock = stock + $quantity WHERE id = $item_id");
    }

    $conn->query("UPDATE purchases SET total_amount = $total_amount WHERE id = $purchase_id");
...
<div class="container mt-4">
    ...
    <!-- Purchase Entry Form (Initially Hidden) -->
    ...
    <!-- Purchases List with Pagination -->
...
$(document).ready(function() {
    ...
    $("#toggleForm").click(function() {
        $("#purchaseFormContainer").slideDown();
        $('html, body').animate({ scrollTop: $("#purchaseFormContainer").offset().top }, 400);
    });
    $("#closeForm").click(function() {
        $("#purchaseFormContainer").slideUp();
    });
