...
// Calculate totals
$product_total = array_sum(array_column($details, 'total'));
...
<head>
    ...
    /* Same enhanced print styles for direct access */
    @media print {
        * { visibility: hidden; }
    ...
    <div id="purchaseFormContainer" class="card p-4 shadow mb-4" style="display:<?= ($action === 'edit') ? 'block' : 'none' ?>;">
        ...
        <form method="POST" id="purchaseForm" action="purchase_entry.php?action=<?= ($action === 'edit') ? 'update&id=' . intval($_GET['id']) : 'add' ?>">
            ...
            <div class="form-check form-switch mb-3">
                <input class="form-check-input" type="checkbox" role="switch" name="paid" id="paidToggle" <?= $paid ? 'checked' : '' ?>>
                <label class="form-check-label" for="paidToggle">Paid (Auto-add receipt to supplier ledger)</label>
            </div>
...
<div id="previewContent">
    <div class="print-container">
        ...
        <div class="total-section">
            ...
            <p>&nbsp;&nbsp;Hamali: ₹<?= number_format($purchase['hamali'] ?? 0, 2) ?></p>
            <p>&nbsp;&nbsp;Motar Bhade: ₹<?= number_format($purchase['freight'] ?? 0, 2) ?></p>
            <p>&nbsp;&nbsp;Uchal: ₹<?= number_format($purchase['uchal'] ?? 0, 2) ?></p>
...
document.addEventListener("DOMContentLoaded", function() {
    // Auto-print for direct access (matches preview isolation)
    setTimeout(function() {
        window.print();
