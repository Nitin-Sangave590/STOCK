...
$items = $conn->query("SELECT id, name, purchase_rate FROM items ORDER BY name ASC");
...
if ($action === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "INSERT INTO purchases (supplier_id, invoice_number, purchase_date, hamali, freight, total_amount)
    ...
    if (!empty($_POST['item_id'])) {
        for ($i = 0; $i < count($_POST['item_id']); $i++) {
            ...
            $sqlDetail = "INSERT INTO purchase_details (purchase_id, item_id, quantity, weight, rate, total)
            ...
            // Update item stock
            if (!$conn->query("UPDATE items SET stock = stock + $quantity WHERE id = $item_id")) {
