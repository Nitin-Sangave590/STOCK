...
if ($action === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // $stmt->close();
    $query = "INSERT INTO sales (customer_id, invoice_number, sale_date, hamali, freight, total_amount) 
          VALUES (?, ?, ?, ?, ?, ?)";
    ...
    if (!$stmt) {
        ...
        die("Database prepare failed. Check logs for details.");
    }
            ...
            <tbody>
                ...
                $stmt = $conn->prepare("
                    SELECT 
                        ...
                        COALESCE((SELECT SUM(weight) FROM purchase_details WHERE item_id = i.id), 0) AS total_weight,
