...
<head>
    ...
    <style>
        ...
        /* Enhanced Print Styles - Isolates preview content exactly as in modal */
        @media print {
            * { visibility: hidden; }
            #previewContent, #previewContent * { visibility: visible; }
            #previewContent { position: absolute; left: 0; top: 0; width: 100%; height: 100%; margin: 0; padding: 0; background: white; }
            ...
            .modal { display: block !important; visibility: visible !important; position: static !important; }
            .modal-backdrop { display: none !important; }
            .modal-header, .modal-footer { display: none !important; }
            body { background: white !important; padding: 0 !important; margin: 0 !important; }
            .container, .card, .pagination, button, .btn, nav, h2, h4 { display: none !important; visibility: hidden !important; }
        }
    </style>
...
<div class="modal fade" id="successModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <a href="Print_purchase.php?page=<?= $page ?>" class="btn btn-primary">Back to Patti List</a>
            </div>
        </div>
    </div>
</div>
...
$(document).ready(function() {
    ...
    // Bulk Print (opens preview first)
    ...
    function loadPreview(ids, action) {
        ...
        $.post('Print_purchase.php', postData, function(response) {
            let $tempDiv = $('<div>').html(response);
            $('#previewContent').html($tempDiv.find('#previewContent').html() || response); // Fallback if no #previewContent
            $('#printPreviewModal').modal('show');
            $('#printBtn').prop('disabled', false);
        });
    }

    // Print Now - Enhanced to ensure modal visibility before print
    $('#printNowBtn').click(function() {
        // Temporarily make modal full-page for print
        $('.modal-dialog').addClass('modal-xl-fullscreen');
        $('.modal').css({ 'z-index': 9999, 'position': 'fixed' });
        ...
        setTimeout(function() {
            ...
            // Reset modal styles
            $('.modal-dialog').removeClass('modal-xl-fullscreen');
            $('.modal').css({ 'z-index': '', 'position': '' });
    ...
    $('#whatsappBtn').click(function() {
        ...
        $.post('Print_purchase.php', { selected_ids: selectedIds, whatsapp: true, start_date: $('[name="start_date"]').val(), end_date: $('[name="end_date"]').val(), page: <?= $page ?> }, function(response) {
    ...
    // Add fullscreen class for modal if needed
    const style = document.createElement('style');
    style.innerHTML = `
        .modal-xl-fullscreen .modal-dialog { max-width: 100% !important; margin: 0 !important; }
        .modal-xl-fullscreen .modal-content { border-radius: 0 !important; height: 100vh !important; }
    `;
