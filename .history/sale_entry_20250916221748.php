...
$stmt = $conn->prepare("INSERT INTO sales (customer_id, invoice_number, `date`, hamali, freight, total_amount)
                            VALUES (?, ?, ?, ?, ?, ?)");
...
$stmt = $conn->prepare("SELECT s.*, a.name AS customer FROM sales s JOIN accounts a ON s.customer_id = a.id ORDER BY s.id DESC LIMIT ? OFFSET ?");
                ...
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>
                        ...
                        <td><a href='.php?id={print_purchase$row['id']}' target='_blank' class='btn btn-sm btn-outline-primary'>🖨 Print</a></td>
                    </tr>";
                }
                $stmt->close();
