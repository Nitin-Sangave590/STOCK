...
<div class="container my-4">
    ...
    <div class="table-responsive">
        <table class="table table-striped table-bordered">
            ...
            $sql = "
                SELECT 
                    COALESCE(p.purchase_date, s.date) AS report_date,
                    COALESCE(SUM(p.total_amount), 0) AS purchase_amount,
                    COALESCE(SUM(s.total_amount), 0) AS sale_amount
                FROM (
                    SELECT purchase_date, SUM(total_amount) AS total_amount
                    FROM purchases
                ...
                ON p.purchase_date = s.date
                GROUP BY COALESCE(p.purchase_date, s.date)
                ORDER BY COALESCE(p.purchase_date, s.date) ASC
            ";
