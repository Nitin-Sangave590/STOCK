...
$stmt = $conn->prepare("SELECT a.id, a.name, 
  COALESCE( (SELECT SUM(p.total_amount) FROM sales p WHERE c.customer_id = a.id), 0) - 
  COALESCE( (SELECT SUM(sp.amount) FROM customer_payments sp WHERE sp.customer_id = a.id), 0) AS balance 
FROM accounts a WHERE a.type = ? ORDER BY a.name DESC");
$type = 'customer';
$stmt->bind_param("c", $type);
$stmt->execute();
