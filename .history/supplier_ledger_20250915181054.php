<?php
include 'db.php'; // ✅ db.php is always needed
...
if (isset($_GET['ajax']) && $_GET['ajax'] == '1') {
    ...
    if (!$purchase_date_col) { die("No date-like column found in purchases table. Run: DESCRIBE purchases;"); }

    // detect supplier receipts table date & description column names
    $receipts_table = 'supplier_receipts';
    ...
    $purchases_sql = "SELECT invoice_number, bill_no, total_amount AS amount, `{$purchase_date_col}` AS date, hamali, freight
                      FROM purchases
                      WHERE supplier_id = ? AND `{$purchase_date_col}` BETWEEN ? AND ?
                      ORDER BY `{$purchase_date_col}` ASC";
    $stmt1 = $conn->prepare($purchases_sql);
    if (!$stmt1) { die("SQL Prepare Error (Purchases): " . $conn->error); }
    $stmt1->bind_param("iss", $supplier_id, $from, $to);
    $stmt1->execute();
    $purchases = $stmt1->get_result();

    // Prepare receipts query (Credit)
    ...
    $receipts_sql = "SELECT amount, `{$receipts_date_col}` AS date, {$desc_sel}
                     FROM `{$receipts_table}`
                     WHERE supplier_id = ? AND `{$receipts_date_col}` BETWEEN ? AND ?
...
if (!$suppliers) { die("SQL Error fetching suppliers: " . $conn->error); }
?>
<!doctype html>
<html>
<head>
  ...
  <style>
    ...
    .modal-ledger .modal-content { border-radius:14px; border:3px solid #198754; }
    .modal-ledger .modal-header { background:#198754; color:white; border-top-left-radius:11px; border-top-right-radius:11px; }
    .modal-legend { font-weight:600; color:#198754; }
  </style>
  <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
</head>
<body>
  <h2 class="mb-4">Supplier Ledger</h2>
    ...
    <table class="table table-hover mb-0">
      ...
      <tbody>
        <?php while ($c = $suppliers->fetch_assoc()):
            $sid = (int)$c['id'];
            $r1 = $conn->query("SELECT COALESCE(SUM(total_amount),0) AS p FROM purchases WHERE supplier_id={$sid}");
            $total_purchases = $r1 ? floatval($r1->fetch_assoc()['p']) : 0;
            $r2 = $conn->query("SELECT COALESCE(SUM(amount),0) AS r FROM supplier_receipts WHERE supplier_id={$sid}");
            $total_payments = $r2 ? floatval($r2->fetch_assoc()['r']) : 0;
            $balance = $total_purchases - $total_payments;
        ?>
          <tr>
            <td><?= htmlspecialchars($c['name']) ?></td>
            <td class="text-end <?= $balance >= 0 ? 'text-danger' : 'text-success' ?>"><?= number_format($balance,2) ?></td>
