...
$sales_sums = [];
if (!empty($ids_str)) {
    $sql = "SELECT customer_id, COALESCE(SUM(total_amount), 0) AS total_sales FROM sales WHERE customer_id IN ({$ids_str}) GROUP BY customer_id";
    $res = $conn->query($sql);
    ...
    $sql2 = "SELECT customer_id, COALESCE(SUM(amount), 0) AS total_receipt FROM customer_receipt WHERE customer_id IN ({$ids_str}) GROUP BY customer_id";
    $res2 = $conn->query($sql2);
    while ($r = $res2->fetch_assoc()) {
        $receipt_sums[$r['customer_id']] = floatval($r['total_receipt']);
    }
} else {
    $customers_list = [];
    $sales_sums = [];
...
// Calculate total balance of all customers combined
$total_all_balance = 0.0;
foreach ($customers_list as $customer) {
    $sid = (int)$customer['id'];
    $total_sales = $sales_sums[$sid] ?? 0.0;
    $total_payments = $receipts_sums[$sid] ?? 0.0;
    $balance = $total_purchases - $total_receipt;
...
<body>
  ...
  <div class="table-responsive ledger-table p-3">
    ...
    <div class="text-center fs-5 fw-bold" id="supp_total">                            
                            Total Balance: 
