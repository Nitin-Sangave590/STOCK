            ...
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    $query = "
                        ...
                        LEFT JOIN purchase_details pd_filter ON pd.purchase_id = pd_filter.purchase_id
                        LEFT JOIN purchases p ON pd.purchase_id = p.id
                        LEFT JOIN sale_details sd ON i.id = sd.item_id
                        LEFT JOIN sale_details sd_filter ON sd.sale_id = sd_filter.sale_id
                    ...
                    if (!empty($params)) {
                        ...
                        array_unshift($param_refs, $param_types);
                        call_user_func_array([$stmt, 'bind_param'], $param_refs);
                    }
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        $totals['invested_amount'] += $row['invested_amount'];
                    ...
                    // Calculate average rates for totals (weighted by weight if applicable)
                    $totals['avg_purch_rate'] = $totals['total_purch_weight'] > 0 ? $totals['avg_purch_rate'] / $result->num_rows : 0;
                    $totals['last_avg_purch_rate'] = $result->num_rows > 0 ? $totals['last_avg_purch_rate'] / $result->num_rows : 0;
                    $totals['avg_sale_rate'] = $totals['total_sold_weight'] > 0 ? $totals['avg_sale_rate'] / $result->num_rows : 0;
                    $totals['last_avg_sale_rate'] = $result->num_rows > 0 ? $totals['last_avg_sale_rate'] / $result->num_rows : 0;
                    $totals['avg_rate'] = $totals['remaining_weight'] > 0 ? $totals['invested_amount'] / $totals['remaining_weight'] : 0;
                    ?>
                </tbody>
                <tfoot>
                    <tr class="footer-row">
                        ...
                        <td><strong><?= number_format($totals['invested_amount'], 2) ?></strong></td>
