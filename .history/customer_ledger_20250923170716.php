...
$sales_sums = [];
if (!empty($ids_str)) {
    $sql = "SELECT customer_id, COALESCE(SUM(total_amount), 0) AS total_purchase FROM sales WHERE customer_id IN ({$ids_str}) GROUP BY customer_id";
    $res = $conn->query($sql);
    ...
    $sql2 = "SELECT customer_id, COALESCE(SUM(amount), 0) AS total_receipts FROM customer_receipts WHERE customer_id IN ({$ids_str}) GROUP BY customer_id";
    $res2 = $conn->query($sql2);
    while ($r = $res2->fetch_assoc()) {
        $receipts_sums[$r['customer_id']] = floatval($r['total_receipts']);
    }
} else {
    $customers_list = [];
    $sales_sums = [];
