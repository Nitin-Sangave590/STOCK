...
if (!empty($ids_str)) {
    ...
    $sql2 = "SELECT customer_id, COALESCE(SUM(amount), 0) AS total_receipts FROM customer_receipt WHERE customer_id IN ({$ids_str}) GROUP BY customer_id";
...
foreach ($customers_list as $customer) {
    ...
    $total_receiptss = $receipts_sums[$sid] ?? 0.0;
    $balance = $total_sales - $total_receiptss;
    ...
    <table class="table table-hover mb-0">
      ...
      <tbody>
        <?php while ($c = $customers->fetch_assoc()):
            ...
            $total_receiptss = $r2 ? floatval($r2->fetch_assoc()['r']) : 0;
            $balance = $total_sales - $total_receiptss;
        ?>
          <tr>
