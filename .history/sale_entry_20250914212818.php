...
if (!$stmt) die("Failed to prepare statement for customers: " . $conn->error);
$type = 'customer';
...
if (!$stmt) die("Failed to prepare statement for items: " . $conn->error);
$stmt->execute();
...
if ($action === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sale_date = $_POST['date']; // renamed variable
    ...
    // ✅ INSERT into sales (FIXED COLUMN NAME)
    $stmt = $conn->prepare("INSERT INTO sales (customer_id, invoice_number, sale_date, hamali, freight, total_amount) 
                            VALUES (?, ?, ?, ?, ?, ?)");
    if (!$stmt) {
    ...
    if (!empty($_POST['item_id'])) {
        foreach ($_POST['item_id'] as $i => $item_id) {
            ...
            // Insert details
            $stmt = $conn->prepare("INSERT INTO sale_details (sale_id, item_id, quantity, weight, rate, total) 
                                    VALUES (?, ?, ?, ?, ?, ?)");
