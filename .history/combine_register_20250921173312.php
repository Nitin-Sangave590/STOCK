...
$sql = "SELECT t.transaction_id, t.type, t.invoice_number, t.transaction_date, a.name as account_name, i.name as item_name, 
        t.weight, t.quantity, t.rate, t.hamali, t.freight, t.total_amount
        FROM (
            SELECT p.id as transaction_id, 'purchase' as type, p.invoice_number, p.purchase_date as transaction_date, p.supplier_id as account_id, 
                   pd.item_id, pd.weight, NULL as quantity, pd.rate, p.hamali, p.freight, p.total_amount
            FROM purchases p
            INNER JOIN purchase_details pd ON p.id = pd.purchase_id
            UNION ALL
            SELECT s.id as transaction_id, 'sale' as type, s.invoice_number, s.date as transaction_date, s.customer_id as account_id, 
                   sd.item_id, sd.weight, sd.quantity, sd.rate, 0 as hamali, 0 as freight, s.total_amount
            FROM sales s
...
$sql .= " ORDER BY t.transaction_date DESC, t.transaction_id DESC LIMIT ?, ?";
