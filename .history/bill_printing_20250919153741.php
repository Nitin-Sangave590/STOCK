<?php
// sale_bill_printing.php - Sale Bill Printing with A5 Page Size Support
ob_start();
include 'db.php';
include 'header.php';
...
$columns = ["s.id", "s.invoice_number", "s.sale_date", "c.name AS customer"];
...
if (columnExists($conn, "sales", "discount")) $columns[] = "s.discount";
if (columnExists($conn, "sales", "tax")) $columns[] = "s.tax";
...
if (isset($_POST['preview']) || isset($_POST['print']) || isset($_POST['single_preview'])) {
    ...
    if (!empty($selected_ids)) {
        ...
        $sql_details = "SELECT s.*, c.name AS customer, sd.quantity, sd.rate, sd.total, i.name AS item_name 
                        FROM sales s 
                        JOIN customers c ON s.customer_id = c.id 
        ...
        if ($stmt_details) {
            ...
            foreach ($selected_ids as $id) {
                ...
                $tax = $sale['tax'] ?? 0;
                $net_amount = $product_total - $discount + $tax;
                ...
                $preview_content .= '<th>Product</th><th>Qty</th><th>Rate</th><th>Total</th>';
                ...
                $preview_content .= '<p>&nbsp;&nbsp;Discount: ₹' . number_format($discount, 2) . '</p>';
                $preview_content .= '<p>&nbsp;&nbsp;Tax: ₹' . number_format($tax, 2) . '</p>';
    ...
    if (!empty($selected_ids)) {
        ...
        if ($stmt_details) {
            ...
            foreach ($selected_ids as $id) {
                ...
                foreach ($sale_details as $detail) {
                    $whatsapp_msg .= "Product: " . htmlspecialchars($detail['item_name']) . ", Qty: " . $detail['quantity'] . ", Rate: ₹" . number_format($detail['rate'], 2) . ", Total: ₹" . number_format($detail['total'], 2) . "\n";
                }
                $product_total = array_sum(array_column(array_values($sale_details), 'total'));
                $discount = $sale['discount'] ?? 0;
                $tax = $sale['tax'] ?? 0;
                ...
                $whatsapp_msg .= "Discount: ₹" . number_format($discount, 2) . "\n";
                $whatsapp_msg .= "Tax: ₹" . number_format($tax, 2) . "\n";
...
<head>
    ...
    <title>Sale Bill Register</title>
...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2 class="fw-bold">Sale Bill Register</h2>
    </div>
...
<div class="modal fade no-print" id="successModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <a href="sale_bill_printing.php?page=<?= $page ?>" class="btn btn-primary">Back to Sale Bill List</a>
            </div>
        </div>
    </div>
</div>
...
$(document).ready(function() {
    ...
    function loadPreview(ids, action) {
        ...
        $.post('sale_bill_printing.php', postData, function(response) {
    ...
    $('#whatsappBtn').click(function() {
        ...
        $.post('sale_bill_printing.php', { selected_ids: selectedIds, whatsapp: true, start_date: $('[name="start_date"]').val(), end_date: $('[name="end_date"]').val(), page: <?= $page ?> }, function(response) {
