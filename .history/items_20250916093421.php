<?php
// items.php - Item Master with Add + Edit functionality
include 'db.php';
include 'header.php';
...
if ($action === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($name)) {
        ...
        if ($stmt->execute()) {
            ...
            echo "<p class='text-danger text-center'>Error adding item: " . $stmt->error . "</p>";
        }
        $stmt->close();
    } else {
...
if ($action === 'edit' && $_SERVER['REQUEST_METHOD'] === 'POST' && $id > 0) {
    ...
    if (!empty($name)) {
        ...
        if ($stmt->execute()) {
            ...
            echo "<p class='text-danger text-center'>Error updating item: " . $stmt->error . "</p>";
        }
        $stmt->close();
    } else {
        echo "<p class='text-danger text-center'>Name is required!</p>";
...
<div class="container">
    ...
    <?php if ($action === 'add' || $action === 'edit'): ?>
        ...
        <form method="POST" action="items.php?action=<?= $action ?><?= $id > 0 ? "&id={$id}" : "" ?>" class="mb-4">
            <div class="mb-3">
                <label>Name</label>
