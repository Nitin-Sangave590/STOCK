...
if ($action === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$supplier_id || !$invoice_number || !$purchase_date) {
        ...
        if (!$stmt->execute()) {
            ...
            if ($modal_type !== 'error') {
                $net_amount = $gross_total - $hamali - $freight - $uchal; // Net amount after deductions
                $stmt = $conn->prepare("UPDATE purchases SET total_amount = ? WHERE id = ?");
                $stmt->bind_param("di", $net_amount, $purchase_id);
                if ($stmt->execute()) {
...
<div class="container mt-4">
    ...
    <div id="purchaseFormContainer" class="card p-4 shadow mb-4" style="display:<?= ($action === 'edit') ? 'block' : 'none' ?>;">
        <form method="POST" id="purchaseForm" action="purchase_entry.php?action=<?= ($action === 'edit') ? 'update&id=' . intval($_GET['id']) : 'add' ?>">
            ...
            <!-- Gross Total and Net Amount -->
            <div class="row mb-3">
                <div class="col-md-6">
                    <label class="form-label">Gross Total</label>
                    <input type="number" step="0.01" id="grossTotal" class="form-control fw-bold" readonly>
                </div>
                <div class="col-md-6">
...
$(document).ready(function() {
    ...
    function calculateTotals() {
        ...
        let netAmount = grossTotal - hamali - freight - uchal;
