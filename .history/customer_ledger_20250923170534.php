...
$sales_sums = [];
if (!empty($ids_str)) {
    $sql = "SELECT customer_id, COALESCE(SUM(total_amount), 0) AS total_purchase FROM purchases WHERE customer_id IN ({$ids_str}) GROUP BY customer_id";
    $res = $conn->query($sql);
    while ($r = $res->fetch_assoc()) {
        $purchase_sums[$r['customer_id']] = floatval($r['total_purchase']);
    }

    // Get total payments per customer
    $sql2 = "SELECT customer_id, COALESCE(SUM(amount), 0) AS total_payment FROM customer_payments WHERE customer_id IN ({$ids_str}) GROUP BY customer_id";
    $res2 = $conn->query($sql2);
    while ($r = $res2->fetch_assoc()) {
        $payment_sums[$r['customer_id']] = floatval($r['total_payment']);
    }
} else {
    $customers_list = [];
    $purchase_sums = [];
