<?php
// customer_receipt_printing.php - Customer Receipt Printing with A5 Page Size Support
ob_start();
include 'db.php';
include 'header.php';
...
$columns = ["cr.id", "cr.receipt_date", "cr.amount", "cr.description", "a.name AS customer"];
if (columnExists($conn, "customer_receipts", "sale_id")) {
    $columns[] = "cr.sale_id";
}
...
$sql = "SELECT " . implode(", ", $columns) . " 
        ...
        JOIN accounts a ON cr.customer_id = a.id 
        WHERE 1=1";
...
$sql .= " ORDER BY cr.id DESC LIMIT ? OFFSET ?";
...
if (isset($_POST['preview']) || isset($_POST['print']) || isset($_POST['single_preview'])) {
    ...
    if (isset($_POST['single_preview'])) {
        ...
        $selected_ids = array_column($receipts, 'id');
    }

    if (!empty($selected_ids)) {
        $placeholders = implode(',', array_fill(0, count($selected_ids), '?'));
        $sql_details = "SELECT cr.*, a.name AS customer 
                        FROM customer_receipts cr 
                        JOIN accounts a ON cr.customer_id = a.id 
        ...
        if ($stmt_details) {
            ...
            foreach ($selected_ids as $id) {
                $receipt = array_filter($details, function($d) use ($id) { return $id == $d['id']; });
                $receipt = array_values($receipt)[0];
                ...
                $preview_content .= '<h5>Customer Receipt</h5>';
                $preview_content .= '</div>';

                $preview_content .= '<div class="invoice-details">';
                $preview_content .= '<p><strong>Customer:</strong> ' . htmlspecialchars($receipt['customer']) . '</p>';
                $preview_content .= '<p><strong>Receipt No:</strong> REC-' . htmlspecialchars($receipt['id']) . '</p>';
                $preview_content .= '<p><strong>Date:</strong> ' . date("d-m-Y", strtotime($receipt['receipt_date'])) . '</p>';
                $preview_content .= '</div>';
                ...
                $preview_content .= '<tr>';
                $preview_content .= '<td>₹' . number_format($receipt['amount'], 2) . '</td>';
                $preview_content .= '<td>' . htmlspecialchars($receipt['description'] ?: 'No description provided') . '</td>';
                $preview_content .= '</tr>';
                $preview_content .= '</tbody></table>';
                $preview_content .= '</div>';

                $preview_content .= '<div class="total-section">';
                $preview_content .= '<p><strong>Total Amount:</strong> ₹' . number_format($receipt['amount'], 2) . '</p>';
                if ($receipt['sale_id']) {
                    $preview_content .= '<p><strong>Linked Sale ID:</strong> #' . $receipt['sale_id'] . '</p>';
                }
                $preview_content .= '</div>';
...
if (isset($_POST['whatsapp'])) {
    ...
    if (!empty($selected_ids)) {
        ...
        if ($stmt_details) {
            ...
            $whatsapp_msg = "Selected Customer Receipt Details:\n";
            foreach ($selected_ids as $id) {
                $receipt = array_filter($details, function($d) use ($id) { return $id == $d['id']; });
                $receipt = array_values($receipt)[0];

                $whatsapp_msg .= "Receipt No: REC-" . htmlspecialchars($receipt['id']) . "\n";
                $whatsapp_msg .= "Date: " . date("d-m-Y", strtotime($receipt['receipt_date'])) . "\n";
                $whatsapp_msg .= "Customer: " . htmlspecialchars($receipt['customer']) . "\n";
                $whatsapp_msg .= "Amount: ₹" . number_format($receipt['amount'], 2) . "\n";
                $whatsapp_msg .= "Description: " . htmlspecialchars($receipt['description'] ?: 'No description provided') . "\n";
                if ($receipt['sale_id']) {
                    $whatsapp_msg .= "Linked Sale ID: #" . $receipt['sale_id'] . "\n";
                }
                $whatsapp_msg .= "----------------\n";
            }
...
// Pagination
$stmt = $conn->prepare("SELECT COUNT(*) as total FROM customer_receipts");
$stmt->execute();
...
<head>
    ...
    <title>Customer Receipt Register</title>
...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2 class="fw-bold">Customer Receipt Register</h2>
    </div>
    ...
    <div class="card shadow p-3">
        ...
        <h4 class="fw-bold mb-3">📋 Customer Receipt List</h4>
...
<div class="modal fade no-print" id="successModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <a href="customer_receipt_printing.php?page=<?= $page ?>" class="btn btn-primary">Back to Receipt List</a>
            </div>
        </div>
    </div>
</div>
...
$(document).ready(function() {
    ...
    $('.single-preview-btn').click(function() {
        ...
        loadPreview([receiptId], 'single_preview');
    });
    ...
    function loadPreview(ids, action) {
        ...
        $.post('customer_receipt_printing.php', postData, function(response) {
    ...
    $('#whatsappBtn').click(function() {
        ...
        $.post('customer_receipt_printing.php', { selected_ids: selectedIds, whatsapp: true, start_date: $('[name="start_date"]').val(), end_date: $('[name="end_date"]').val(), page: <?= $page ?> }, function(response) {
